...
def dashboard():
    ...
    rides = (
        ...
        .order_by(Ride.departure_time.asc()) # Show soonest rides first
        .all()
    )
...
def ping_driver(ride_id):
    ...
    # 1. Save the ping to the chat history
    ...
    # 2. Emit to the specific chat room (if they are on the chat page)
    socketio.emit(
        "new_message",
        {
            "ride_id": ride_id,
    ...
    # 3. NEW: Emit a global notification to find the driver wherever they are
    socketio.emit(
        "global_ping",
        {
            "driver_id": ride.driver_id,
            "message": f"🔔 {user.name} just pinged you regarding your ride to {ride.destination}!"
        }
    )
